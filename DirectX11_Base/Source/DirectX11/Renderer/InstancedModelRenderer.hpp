...
private:
	...
	std::vector<std::shared_ptr<ShaderParam>> m_pShaderParamsPS; // シェーダーパラメータ
	std::vector<std::shared_ptr<ShaderParam>> m_pShaderParamsVS; // シェーダーパラメータ
};
...
template<typename T>
requires std::derived_from<T, ShaderParam>
inline void InstancedModelRenderer::SetWriteParamForPS(_In_ const std::shared_ptr<T> &In_Param)
{
	if (In_Param)
...
template<typename T>
requires std::derived_from<T, ShaderParam>
inline void InstancedModelRenderer::SetWriteParamForVS(const std::shared_ptr<T> &In_Param)
{
	if (In_Param)
