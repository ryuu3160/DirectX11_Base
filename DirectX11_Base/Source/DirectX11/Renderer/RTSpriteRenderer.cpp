/*+===================================================================
	File: RTSpriteRenderer.cpp
	Summary: レンダーターゲットのスプライトレンダラー
	Author: AT13C192 01 青木雄一郎
	Date: 2025/11/13 Thu AM 10:41:14 初回作成
===================================================================+*/
...
RTSpriteRenderer::RTSpriteRenderer()
	: RenderComponent("RTSpriteRenderer")
	, m_bIsLoaded(false), m_bUsePixelPosition(false), m_PositionPixel{}
{
	...
	GetPositionPixel();
	m_LayerGroup = LayerGroup_RenderTexture;
}
...
void RTSpriteRenderer::SetVertexShader(_In_ Shader *In_Vs) noexcept
{
	if (NullCheck(In_Vs, NCMode::OUTPUT, "VertexShader is null in RTSpriteRenderer::SetVertexShader."))
		return;
	m_SpriteData.vs = In_Vs;
}

void RTSpriteRenderer::SetPixelShader(_In_ Shader *In_Ps) noexcept
{
	if (NullCheck(In_Ps, NCMode::OUTPUT, "PixelShader is null in RTSpriteRenderer::SetPixelShader."))
		return;
	m_SpriteData.ps = In_Ps;
}

void RTSpriteRenderer::SetRenderContext(_In_ RenderContext *In_Context) noexcept
{
	m_Context = In_Context;
}

void RTSpriteRenderer::Load(_In_ const FilePath &In_File, _In_ const float &In_Scale)
{
	// 頂点データの作成
	SpriteRenderer::SpriteVertex vtx[] = {
	...
	desc.pVtx = vtx;
	desc.vtxSize = sizeof(SpriteRenderer::SpriteVertex);
	...
	// テクスチャの設定
	m_SpriteData.texture = m_Context->GetRTV();
}

void RTSpriteRenderer::Load() noexcept
{
	// 既に読み込まれている場合は何もしない
	if (m_bIsLoaded)
		return;
...
void RTSpriteRenderer::Draw(_In_ RenderContext *In_RenderContext) noexcept
{
	// 2D描画の準備
	...
	DirectX::XMMATRIX R = DirectX::XMMatrixRotationQuaternion(DirectX::XMVectorSet(Quat.x, Quat.y, Quat.z, Quat.w));
	...
	auto RTV = m_Context->GetRTV();
	if (RTV)
		m_SpriteData.texture = RTV;
	else
	...
	m_SpriteData.ps->SetTexture(0, nullptr); // テクスチャのバインド解除
}

const DirectX::XMFLOAT3 &RTSpriteRenderer::GetPositionPixel() noexcept
{
	if (!m_pGameObject)
...
void RTSpriteRenderer::SetOffset(_In_ const DirectX::XMFLOAT2 &In_Offset) noexcept
{
	m_SpriteData.param[0].x = In_Offset.x;
	m_SpriteData.param[0].y = In_Offset.y;
}
void RTSpriteRenderer::SetSize(_In_ const DirectX::XMFLOAT2 &In_Size) noexcept
{
...
void RTSpriteRenderer::SetUVPos(_In_ const DirectX::XMFLOAT2 &In_Pos) noexcept
{
	m_SpriteData.param[1].x = In_Pos.x;
	m_SpriteData.param[1].y = In_Pos.y;
}
void RTSpriteRenderer::SetUVScale(_In_ const DirectX::XMFLOAT2 &In_Scale) noexcept
{
	m_SpriteData.param[1].x = In_Scale.x;
	m_SpriteData.param[1].y = In_Scale.y;
}
void RTSpriteRenderer::SetColor(_In_ const DirectX::XMFLOAT4 &In_Color) noexcept
{
	m_SpriteData.param[2] = In_Color;
}

void RTSpriteRenderer::Set3D(_In_ const bool &In_Is3D) noexcept
{
	m_SpriteData.Is3D = In_Is3D;
}
void RTSpriteRenderer::SetBillBoard(_In_ const bool &In_IsBillBoard) noexcept
{
	m_SpriteData.IsBillBoard = In_IsBillBoard;
}

void RTSpriteRenderer::SetPositionPixel(_In_ const DirectX::XMFLOAT3 &In_Pos) noexcept
{
