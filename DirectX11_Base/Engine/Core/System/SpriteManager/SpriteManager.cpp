...
void SpriteManager::SaveSprites() const noexcept
{
	...
	std::string path = "Engine\\Assets\\SpriteManager";
...
{
	if (!std::filesystem::exists("Engine\\Assets\\SpriteManager"))
		std::filesystem::create_directories("Engine\\Assets\\SpriteManager");

	std::ofstream file("Engine\\Assets\\SpriteManager\\Scenes.csv", std::ios::trunc);
...
void SpriteManager::LoadAllScene() noexcept
{
	...
	std::ifstream file("Engine\\Assets\\SpriteManager\\Scenes.csv");
	if (file.is_open())
	{
